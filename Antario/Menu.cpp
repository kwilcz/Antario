...
{
    return vecPointPos.x > vecDst1.x && vecPointPos.x < vecDst2.x && vecPointPos.y > vecDst1.y && vecPointPos.y <
            vecDst2.y;
}
...
BaseWindow::BaseWindow(Vector2D    vecPosition, Vector2D vecSize, CD3DFont* pFont, CD3DFont* pHeaderFont,
                       std::string strLabel)
{
    this->pFont         = pFont;
...
Checkbox::Checkbox(std::string strLabel, bool* bValue)
{
    ...
    this->vecSelectableSize = Vector2D(std::roundf(static_cast<float>(size.cy) * 0.70f),
                                       std::roundf(static_cast<float>(size.cy) * 0.70f));
}
...
void Checkbox::UpdateData()
{
    ...
    if (MenuMain::mouseCursor->IsInBounds(this->vecPosition, (this->vecPosition + this->vecSize)))
    {
        ...
        else if (!this->mouseCursor->bLMBPressed && bIsChanged)
            bIsChanged = false;
...
{
    std::shared_ptr<BaseWindow> mainWindow = std::make_shared<BaseWindow>(Vector2D(450, 450), Vector2D(360, 256),
                                                                          g_Fonts.pFontTahoma8.get(),
                                                                          g_Fonts.pFontTahoma10.get(),
                                                                          "Antario - Main"); // Create our main window (Could have multiple if you'd create vec. for it)
    {
